...
rate = rospy.set_param("/rate/lineDetector", 10)
speed = rospy.set_param("speed", 0.3)
steer_multiplier = rospy.set_param("steering_multiplier", 0.8)
steer_slowdown = rospy.set_param("steering_slowdown", 0.6)
...
class LineDetector:
    def __init__(self):
        ...
        self.image_sub_rpi = rospy.Subscriber("/raspicam_node/image/compressed", CompressedImage, self.image_callback_compressed, queue_size=1)
        self.image_sub = rospy.Subscriber("/camera/color/image_raw", Image, self.image_callback_raw, queue_size=1)
        self.image_pub = rospy.Publisher("processed_image", Image, queue_size=1)
        self.speed_pub = rospy.Publisher('/motor_driver/speed', Float32, queue_size=1) # change it depends on the motor driver script u have 
        self.dir_pub = rospy.Publisher('/motor_driver/direction', Float32, queue_size=1) # change it depends on the motor driver script u have 
    ...
    def process_image(self, cv_image):
        ...
        # Threshold the image
        (T, threshold_image) = cv2.threshold(cv_image, 50, 255, cv2.THRESH_BINARY_INV)
